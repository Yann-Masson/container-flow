...
import {
    listContainers,
    startGenericContainer,
    stopGenericContainer,
    removeGenericContainer,
    duplicateGenericContainer,
} from '@/store/slices/containerSlice';
import configs from '../../../electron/services/docker/configs/index.ts';
...
export const getImageBadgeStyle = (image: string) => {
...
export default function ContainerList() {
    ...
    const handleCreateContainer = async (
        ...
        try {
            ...
            await dispatch(
                duplicateGenericContainer({
                    previousId: previousContainerId,
                    config: containerConfig,
                    removePrevious: removePreviousContainer,
                }),
            ).unwrap();
    ...
    const handleGetLogs = async (
        ...
        try {
            return await dockerClientService.containers.getLogs(
                containerId,
                {
                    stdout: true,
                    stderr: true,
            ...
            return {
                lines: [
                    {
                        original: 'Failed to retrieve logs',
                        cleaned: 'Failed to retrieve logs',
                        category: 'error',
                        timestamp: null,
                    },
                ],
    ...
    const stopped = useMemo(
        () =>
            containers.filter((c) => c.Status?.includes('Exited') || c.Status?.includes('Created'))
                .length,
        [containers],
    );
                ...
                <div className='space-y-4'>
                    ...
                    {status === State.LOADING ? (
                        ...
                        containers.map((container) => {
                            ...
                            const isLoading = !!(
                                ops.starting[container.Id] ||
                                ops.stopping[container.Id] ||
                                ops.removing[container.Id] ||
                                ops.duplicating[container.Id]
                            );
