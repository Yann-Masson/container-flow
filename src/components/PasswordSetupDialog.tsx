...
export default function PasswordSetupDialog({ open, onComplete }: PasswordSetupDialogProps) {
    ...
    const metricsValidation = metricsPassword
        ? validatePassword(metricsPassword)
        : { valid: true, reasons: [] };
    ...
    const canSubmit =
        ...
        (!metricsPassword || metricsValidation.valid) &&
        ...
        <Dialog open={open}>
            <DialogContent className='max-w-md'>
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Provide the MySQL root password (used to manage project databases).
                        Optionally set metrics user credentials.
                    </DialogDescription>
                </DialogHeader>
                <div className='space-y-4 py-2'>
                    <div>
                        <label className='text-sm font-medium'>
                            MySQL Root Password<span className='text-red-500'>*</span>
                        </label>
                        <Input
                            type='password'
                            value={rootPassword}
