...
export function WordPressDeleteDialog({
    projectName,
    open,
    onOpenChange,
}: WordPressDeleteDialogProps) {
    const dispatch = useAppDispatch();
    const operationStatus = useAppSelector(selectOperationStatus);
    const isDeleting = (operationStatus as { deleting?: Record<string, boolean> }).deleting?.[projectName];
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            if (deleteWordPressProject.fulfilled.match(result)) {
                ...
                const errorMsg = (result.payload as string) || 'Failed to delete project';
                toast.error('❌ Error deleting project', { description: errorMsg });
            }
        } catch (e) {
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className='bg-black/80 backdrop-blur-md border-red-500/30 w-xl'>
                <DialogHeader>
                    ...
                    <DialogDescription>
                        This will permanently remove all containers, volumes and the database for{' '}
                        <span className='font-medium text-foreground'>{projectName}</span>. This
