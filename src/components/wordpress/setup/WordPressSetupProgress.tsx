...
import {
    selectWordPressSetupIsRunning,
    selectWordPressSetupSteps,
    runWordPressSetup,
            ...
            {grafanaAuthNeeded && (
                <div className='space-y-3 p-3 border rounded-md'>
                    ...
                    <p className='text-xs text-muted-foreground'>
                        Provisioning failed due to invalid default credentials. Enter the current
                        Grafana admin credentials and retry.
                    </p>
                    <div className='grid grid-cols-1 sm:grid-cols-2 gap-2'>
                        <div className='flex flex-col gap-1'>
                            <label className='text-xs font-medium' htmlFor='grafana-username'>
                                Username
                            </label>
                            <Input
                                id='grafana-username'
                                value={grafanaUsername}
                                onChange={(e) => setGrafanaUsername(e.target.value)}
                                placeholder='admin'
                            />
                        </div>
                        <div className='flex flex-col gap-1'>
                            <label className='text-xs font-medium' htmlFor='grafana-password'>
                                Password
                            </label>
                            <Input
                                id='grafana-password'
                            ...
                            <Tooltip key={step.id}>
                                <TooltipTrigger asChild>
                                    <motion.div
                                        ...
                                        className={`flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 rounded-lg border shadow-sm transition-colors duration-300 w-full max-w-full min-w-0 overflow-hidden ${
                                            step.status === 'running'
                                    ...
                                    >
                                        <div className='flex flex-row items-center gap-3 w-full max-w-full min-w-0 overflow-hidden'>
                                            <div className='flex-shrink-0 hidden min-[450px]:block'>
                                            ...
                                            <div className='flex-grow min-w-0 overflow-hidden w-full'>
                                                <div className='flex flex-col xs:flex-row xs:items-center gap-2 min-w-0 overflow-hidden'>
                                                    <span className='text-gray-900 dark:text-gray-100 overflow-hidden text-ellipsis'>
                                                        {step.label}
                                                    </span>
                                                    <Badge
                                                        variant={getStepBadgeVariant(step.status)}
                                                        className='shrink-0 text-xs whitespace-nowrap hidden sm:inline-flex'
                                                    >
                                        ...
                                        <div className='text-sm text-gray-500 dark:text-gray-400 mt-2 sm:mt-0 sm:ml-4 shrink-0 hidden sm:block'>
                                            {index + 1}
                                        </div>
                                    </motion.div>
                                </TooltipTrigger>
