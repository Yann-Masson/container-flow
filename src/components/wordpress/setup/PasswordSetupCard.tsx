...
export default function PasswordSetupCard({ onComplete }: PasswordSetupCardProps) {
    ...
    const canSubmit =
        rootValidation.valid && metricsValidation.valid && rootPassword && metricsPassword;

    useEffect(() => {
        (async () => {
            try {
                const s = await window.electronAPI.passwords.status();
                if (!s.initialized) {
                    await window.electronAPI.passwords.discover();
                }
                const refreshed = await window.electronAPI.passwords.status();
    ...
    const handleSubmit = async () => {
        ...
        setSubmitting(true);
        try {
            await window.electronAPI.passwords.setRootAndMetrics({
                rootPassword,
                metricsUser,
                metricsPassword,
            });
    ...
    return (
        ...
        >
            <CardHeader>
                ...
                <CardDescription>
                    Enter the MySQL root password and metrics user password. These are required
                    before setting up WordPress services.
                </CardDescription>
            </CardHeader>
            <CardContent className='pt-4 space-y-6'>
                ...
                <div className='space-y-2'>
                    ...
                    {!metricsValidation.valid && metricsPassword && (
                        <ul className='mt-1 text-xs text-red-500 list-disc list-inside'>
                            {metricsValidation.reasons.map((r) => (
                                <li key={r}>{r}</li>
                            ))}
                        </ul>
                    )}
