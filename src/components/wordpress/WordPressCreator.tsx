...
export default function WordPressCreator() {
    ...
    const isFormValid =
        formData.name.trim() && formData.domain.trim() && validateForm().length === 0;
    ...
    return (
        ...
        >
                ...
                {isLoading ? (
                    ...
                    <>
                        ...
                        <CardDescription className='pb-3'>
                            Create a new WordPress project with its own database and Traefik
                            configuration.
                        </CardDescription>
                    </>
                )}
            ...
            <CardContent className='space-y-4'>
                ...
                {!isLoading && (
                    <div
                        className='flex items-center justify-between border rounded-md p-3 bg-muted/30 cursor-pointer'
                        onClick={() => setShowPreview(!showPreview)}
                    >
                        <div className='flex flex-col'>
                            <span className='text-sm font-medium'>Configuration preview</span>
                            <span className='text-xs text-gray-500'>
                                Toggle to show computed project details
                            </span>
                        </div>
                        <Switch
                            checked={showPreview}
                    ...
                    >
                        ...
                        <header className='flex items-center justify-between px-4 py-3 border-b border-white/10 relative z-10'>
                            <div className='flex items-center gap-2'>
                                ...
                                <h4 className='font-medium text-sm tracking-wide text-white/90'>
                        ...
                        <div className='p-4 space-y-6 relative z-10'>
                            <dl className='grid gap-5 sm:grid-cols-2 text-sm'>
                                <div className='space-y-1.5'>
                                    <dt className='flex items-center gap-2 font-medium text-white/70'>
                                        <span className='inline-flex h-5 w-5 items-center justify-center rounded bg-primary/15 text-primary text-[10px] font-semibold'>
                                            ID
                                        </span>
                                        Project Name
                                    </dt>
                                    <dd className='font-mono text-[11px] bg-white/5 px-2 py-1.5 rounded border border-white/10 text-white/90 tracking-tight'>
                                        {computedName}
                                    </dd>
                                </div>
                                <div className='space-y-1.5'>
                                    ...
                                    <dd className='font-mono text-[11px] bg-white/5 px-2 py-1.5 rounded border border-white/10 text-white/90 break-all'>
                                        https://{computedDomain}
                                    </dd>
                                </div>
                                <div className='space-y-1.5'>
                                    <dt className='flex items-center gap-2 font-medium text-white/70'>
                                    ...
                                    <dd className='font-mono text-[11px] bg-white/5 px-2 py-1.5 rounded border border-white/10 text-white/90'>
                                        {containerName}
                                    </dd>
                                </div>
                                <div className='space-y-1.5'>
                            ...
                            <div className='space-y-2'>
                                <p className='text-xs text-white/60 leading-relaxed'>
                                    These values are generated automatically based on your inputs
                                    and will be used to scaffold the WordPress environment
                                    (containers, database & Traefik labels).
                                </p>
                                {!formData.name && (
                                    <p className='text-xs text-amber-400/90 flex items-center gap-1'>
                                        Fill the project name to see the final resource names.
                                    </p>
                                )}
                            </div>
                        </div>
