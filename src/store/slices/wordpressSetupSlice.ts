...
const baseSteps: Omit<SetupStep, 'status' | 'statusMessage'>[] = [
    ...
    {
        id: 'mysql-metrics-user',
        label: 'Creating MySQL User Metrics for Monitoring',
        icon: 'database',
        description: 'A dedicated user for exporting MySQL metrics.',
    },
    {
        id: 'cadvisor',
    ...
    {
        id: 'node-exporter',
        label: 'Deploying Node Exporter',
        icon: 'cpu',
        description: 'Node Exporter provides hardware and OS metrics.',
    },
    {
        id: 'prometheus',
...
export const runWordPressSetup = createAsyncThunk<
    void,
    { force?: boolean; grafanaAuth?: { username: string; password: string } } | void,
    { state: RootState }
>('wordpressSetup/run', async (arg, { dispatch, getState, rejectWithValue }) => {
...
export const selectWordPressSetupIsRunning = (state: RootState) =>
    state.wordpressSetup.status === 'running';
