    ...
    async (
        {
            ...
            config,
            removePrevious,
        }: { previousId: string; config: ContainerCreateOptions; removePrevious: boolean },
        { rejectWithValue },
    ) => {
        try {
            if (removePrevious) {
                ...
                try {
                    await dockerClientService.containers.stop(previousId);
                } catch {
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(startGenericContainer.fulfilled, (state, action) => {
                ...
                state.containers = state.containers.map((c) =>
                    c.Id === action.payload.id
                        ? ({ ...c, State: 'running', Status: 'Up' })
                        : c,
                );
            })
            .addCase(startGenericContainer.rejected, (state, action) => {
                delete state.operationStatus.starting[(action.meta.arg as { id: string }).id];
                state.error = action.payload as string;
            })
            // stop
            ...
            .addCase(stopGenericContainer.rejected, (state, action) => {
                delete state.operationStatus.stopping[(action.meta.arg as { id: string }).id];
                state.error = action.payload as string;
            })
            // remove
            ...
            .addCase(removeGenericContainer.rejected, (state, action) => {
                delete state.operationStatus.removing[(action.meta.arg as { id: string }).id];
                state.error = action.payload as string;
            })
            // duplicate
            ...
            .addCase(duplicateGenericContainer.fulfilled, (state, action) => {
                ...
                state.containers = state.containers.map((c) =>
                    c.Id === action.payload.previousId ? action.payload.newContainer : c,
                );
            })
            .addCase(duplicateGenericContainer.rejected, (state, action) => {
                delete state.operationStatus.duplicating[(action.meta.arg as { previousId: string }).previousId];
                state.error = action.payload as string;
            });
    },
});
