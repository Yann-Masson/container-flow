...
const variants = {
    ...
    enter: {
        opacity: 1,
        scale: 1,
        transition: { duration: 0.25, ease: [0.16, 0.84, 0.44, 1] as [number, number, number, number] },
    },
    exit: { opacity: 0, scale: 1, transition: { duration: 0.18, ease: [0.4, 0, 1, 1] as [number, number, number, number] } },
} as const;
    ...
    return (
        <AnimatePresence mode='popLayout' initial={false}>
            <motion.div
                ...
                animate='enter'
                exit='exit'
                className='flex-grow w-full h-full flex flex-col justify-center items-center overflow-hidden'
            >
                {render()}
            </motion.div>
        </AnimatePresence>
