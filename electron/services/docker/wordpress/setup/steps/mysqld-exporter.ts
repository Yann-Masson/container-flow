...
export async function ensureMysqlExporter(
    ctx: EnsureContext,
    metricsDsn: string | null,
): Promise<void> {
    ...
    if (existing) {
        ...
        if (!validate.containerConfig(info, exporterConfig)) {
            if (!force) {
                const msg =
                    'mysqld-exporter container exists but has invalid configuration. Use force=true to recreate.';
            ...
            progress?.(
                'mysqld-exporter',
                'starting',
                'Recreating invalid mysqld-exporter container...',
            );
