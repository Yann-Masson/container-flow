...
export async function provisionGrafana(
    ...
    if (dsResp.status === 200) {
        ...
        if (!createResp.ok) {
            throw new Error(
                `[Grafana] Failed to create datasource: ${
                    createResp.status
        ...
        throw new Error(
            '[Grafana] Failed querying datasource: 401 Unauthorized. The admin credentials used are invalid.',
        );
    } else {
    ...
    if (!created && finalInfo.url !== prometheusUrl) {
        ...
        if (!upd.ok) {
            throw new Error(
                `[Grafana] Failed to update datasource: ${upd.status} ${await upd.text()}`,
            );
        }
        updated = true;
