...
export const networkDashboard = {
    ...
    panels: [
        ...
        },
        {
            ...
            targets: [
                {
                    expr: 'sum(rate(node_network_receive_errs_total{device!~"lo|docker.*|veth.*|br.*"}[5m]))',
                    legendFormat: 'rx err',
                    refId: 'A',
                },
                {
                    expr: 'sum(rate(node_network_transmit_errs_total{device!~"lo|docker.*|veth.*|br.*"}[5m]))',
                    legendFormat: 'tx err',
                    refId: 'B',
                },
            ],
